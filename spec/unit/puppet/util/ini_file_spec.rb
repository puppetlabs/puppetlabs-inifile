...
describe Puppet::Util::IniFile do
  ...
  context "when giving it a path to an ini file" do
    before :each do
...
foo=foovalue
  ...
  context "when giving it the ini contents directly" do
    let(:sample_content) {
      template = <<-EOS
# This is a comment
[section1]
...
baz=bazvalue
 ...
 zot = multi word value
    ...
    let(:subject_content) { Puppet::Util::IniFile.new(sample_content.join("\n"), ' = ', false) }

    it "should return the same lines as when it reads a file" do
      subject_content.send(:lines).should == sample_content
    end
