...
describe provider_class do
  ...
  context "when working with an ini file as source parameter" do
    context "when given a list of sources" do
      let(:resource) { Puppet::Type::Ini_setting.new({
        :title => 'ini_setting_multi_source_iterate_test',
        :source => ['puppet:///modules/test/file1.ini', 'puppet:///modules/test/file2.ini', 'puppet:///modules/test/file3.ini', 'puppet:///modules/test/file4.ini']
      })}

      it "should iterate over each source and stop at the first non-empty one" do
        dummy = Object.new
        def dummy.content
          <<-EOS
# This is a comment
...
subby=bar
 ...
 ; yet another comment
        ...
        resource.stub(:indirection_get_ini_file).and_return(nil, nil, dummy)
        resource.should_receive(:indirection_get_ini_file).exactly(3).times
        resource.eval_generate
      end
    end

    context "when given a single source" do
      let(:resource) { Puppet::Type::Ini_setting.new({
        :title => 'ini_setting_single_source',
        :source => 'puppet:///modules/test/file1.ini'
      })}
      before :each do
        dummy = Object.new
        def dummy.content
...
subby=bar
 ...
 ; yet another comment
        ...
        resource.stub(:indirection_get_ini_file).and_return(dummy)
      end

      it "should generate as many children ini_setting rules as there are ini options" do
        resource.eval_generate.size.should == 7
      end

      it "should generate a matching ini_setting rule" do
        resource.eval_generate.size.should > 0
        resource.eval_generate[0][:section].should == 'section1'
        resource.eval_generate[0][:setting].should == 'foo'
        # Generated sections should not have a source parameter
        resource.eval_generate[0][:source].size.should == 0
      end
    end
  end

  context "when working with a single ini rule" do
